...
export class ContactDashboardComponent implements OnInit, OnDestroy {
  ...
  saveFromModal(): void {
    console.log('saveFromModal chamado');
    console.log('formData antes de validar:', this.formData);
    ...
    if (!name || !phone) {
      console.log('Validação falhou:', { name, phone, company });
    ...
    if (!(digits === 10 || digits === 11)) {
      globalThis.alert('Phone inválido. Informe DDD + número com 10 ou 11 dígitos.');
      return;
    }
